...
if ($trackText -notmatch 'export interface TrackSummary') {
    throw "Expected TrackSummary type was not found."
}

$enrollmentTypePath = Join-Path $FrontendRoot "src\types\enrollment.ts"
...
# ------------------------------------------------------------
# 2. Payment service â€” FormData is explicit for receipt upload
# ------------------------------------------------------------
$paymentService = @'
import { api } from "./api";
import type {
...
if ($trackText -notmatch 'is_premium\s*:') {
    $trackText = $trackText.Replace(
        "  ordering: number;`r`n  modules?: TrackModule[];",
        "  ordering: number;`r`n  price: number;`r`n  currency: string;`r`n  is_premium: boolean;`r`n  modules?: TrackModule[];"
    )
    $trackText = $trackText.Replace(
        "  ordering: number;`r`n}",
        "  ordering: number;`r`n  price: number;`r`n  currency: string;`r`n  is_premium: boolean;`r`n}")
    Write-Utf8NoBom $trackPath $trackText
}

$enrollmentText = Read-Utf8 $enrollmentTypePath
$enrollmentText = $enrollmentText.Replace(
...
if ($authText -notmatch 'role\??\s*:\s*string') {
    ...
    $authText = $authText.Replace(
        ...
        "  is_superuser: boolean;`r`n  role?: string | null;"
    )
...
if ($trackDetailText -notmatch 'PaymentCheckout') {
    $trackDetailText = $trackDetailText.Replace(
        ...
        'import EnrollmentPanel from "@/components/EnrollmentPanel";`r`nimport PaymentCheckout from "@/components/PaymentCheckout";'
    )
}
...
if ($appText -notmatch 'AdminPayments') {
    $appText = $appText.Replace(
        ...
        'import Tracks from "./pages/Tracks";`r`nimport AdminPayments from "./pages/AdminPayments";'
    )
...
# ------------------------------------------------------------
# 8. Main navigation â€” only admin/superuser sees Payments
# ------------------------------------------------------------
$rootLayoutText = Read-Utf8 $rootLayoutPath
if ($rootLayoutText -notmatch 'AdminPayments') {
    if ($rootLayoutText -notmatch 'useAuth') {
        $rootLayoutText = $rootLayoutText.Replace(
            'import { Outlet, NavLink } from "react-router-dom";',
            'import { Outlet, NavLink } from "react-router-dom";`r`nimport { useAuth } from "@/context/AuthContext";'
        )
        if ($rootLayoutText -notmatch 'useAuth') {
            $rootLayoutText = $rootLayoutText.Replace(
                'import { Outlet } from "react-router-dom";',
                'import { Outlet } from "react-router-dom";`r`nimport { useAuth } from "@/context/AuthContext";'
            )
        }
        if ($rootLayoutText -notmatch 'useAuth') {
