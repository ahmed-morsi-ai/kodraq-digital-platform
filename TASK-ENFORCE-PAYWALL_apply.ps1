...
Write-Host '===== TASK-ENFORCE-PAYWALL AUDIT =====' -ForegroundColor Cyan
...
$createMatch = [regex]::Match(
    $enrollmentsText,
    '(?ms)(?<signature>^def\s+create_enrollment\s*\(.*?\):\s*\r?\n)(?<body>.*?)(?=^\s*@router\.|^def\s+|^class\s+|\z)'
)
...
$curriculumMatch = [regex]::Match(
    $tracksText,
    '(?ms)(?<block>^@router\.get\(\s*["'']/{track_id}/curriculum["''].*?)(?=^\s*@router\.|^def\s+|^class\s+|\z)'
)
...
if ($curriculumBlock -notmatch '\bcurrent_user\b') {
    throw 'Curriculum route does not expose current_user in its implementation; no write performed.'
}
if ($curriculumBlock -notmatch '\btrack\b') {
...
$checkoutText = $checkoutText.Replace('import { enrollmentService } from "@/services/enrollment.service";`r`n', '')
$checkoutText = $checkoutText.Replace('import { enrollmentService } from "@/services/enrollment.service";`n', '')
$checkoutText = $checkoutText.Replace('  onEnrollmentCreated: (enrollment: Enrollment) => void;`r`n', '')
$checkoutText = $checkoutText.Replace('  onEnrollmentCreated: (enrollment: Enrollment) => void;`n', '')
$checkoutText = $checkoutText.Replace('  enrollment,`r`n  onEnrollmentCreated,`r`n}: PaymentCheckoutProps)', '  enrollment,`r`n}: PaymentCheckoutProps)')
$checkoutText = $checkoutText.Replace('  enrollment,`n  onEnrollmentCreated,`n}: PaymentCheckoutProps)', '  enrollment,`n}: PaymentCheckoutProps)')
$checkoutText = $checkoutText.Replace('  const [isEnrollmentSubmitting, setIsEnrollmentSubmitting] = useState(false);`r`n', '')
$checkoutText = $checkoutText.Replace('  const [isEnrollmentSubmitting, setIsEnrollmentSubmitting] = useState(false);`n', '')
...
'@
$checkoutText = $checkoutText.Replace(($oldStartHandler -replace "`n", "`r`n"), '')
$checkoutText = $checkoutText.Replace($oldStartHandler, '')
...
if ($checkoutText.Contains($oldButtonCrLf)) {
    ...
    throw 'PaymentCheckout initial checkout button anchor not found; no write performed.'
}
...
$newEffect = @'
  ...
  }, [currentEnrollment, hasCurriculumAccess, loadAssignments, loadCurriculum, loadProgress]);
'@
...
$learningStart = $trackDetailText.IndexOf('        {!isEnrollmentLoading && (')
$closingMarker = "      </div>`r`n    </div>`r`n  );"
$learningEnd = $trackDetailText.LastIndexOf($closingMarker)
if ($learningStart -lt 0 -or $learningEnd -le $learningStart) {
    throw 'Could not isolate TrackDetail post-header learning block; no write performed.'
}

$existingLearning = $trackDetailText.Substring($learningStart, $learningEnd - $learningStart).TrimEnd()
$existingIndented = ($existingLearning -split "`r?`n" | ForEach-Object {
...
$salesBranch = @'
            <section className="space-y-6">
              ...
              <div className="space-y-4">
                {track.is_premium && !currentEnrollment ? (
                  <EnrollmentPanel
...
'@
$salesBranchCrLf = $salesBranch -replace "`n", "`r`n"

$newLearningBlockCrLf = @'
        {!isEnrollmentLoading && (
          hasCurriculumAccess ? (
            <>
'@
$newLearningBlockCrLf = ($newLearningBlockCrLf -replace "`n", "`r`n")
$newLearningBlockCrLf += $existingIndented + "`r`n            </>`r`n          ) : (`r`n"
$newLearningBlockCrLf += $salesBranchCrLf
$newLearningBlockCrLf += "          )`r`n        )}`r`n"

$trackDetailText = $trackDetailText.Remove($learningStart, $learningEnd - $learningStart).Insert($learningStart, $newLearningBlockCrLf)
...
Write-Host ''
Write-Host 'TASK-ENFORCE-PAYWALL VERIFICATION PASSED.' -ForegroundColor Green
Write-Host 'No git commands executed. No commit created.' -ForegroundColor Yellow
