...
# Clean runner v3. No git commit/push.
...
function Replace-Exact([string]$Text, [string]$Old, [string]$New, [string]$Label) {
    ...
    if ($count -eq 0) { throw "Anchor not found: $Label" }
    if ($count -gt 1) { throw "Anchor not unique: $Label ($count matches)" }
...
foreach ($pair in @(
    @($track, 'class Track(Base, TimestampMixin):')
    @($enrollment, 'class Enrollment(Base, TimestampMixin):')
    @($crudEnrollment, 'def create_for_user(')
    @($enrollmentApi, 'def update_enrollment(')
    @($api, 'enrollments.router')
    @($modelsInit, 'from app.models.track import')
    @($userModel, 'class User(Base, TimestampMixin):')
)) {
    if (-not $pair[0].Contains($pair[1])) { throw "Audit failed: $($pair[1])" }
}
...
if (Test-Path $paymentPath) {
    $existing = Read-Lf $paymentPath
    if ($existing.Trim()) { throw "app/models/payment.py already exists and is non-empty. Refusing overwrite." }
}
...
# ---------------------------------------------------------------------------
# Premium enrollment creation gate.
# ---------------------------------------------------------------------------
$crudEnrollment = Read-Lf $crudEnrollmentPath
...
# ---------------------------------------------------------------------------
# Prevent premium activation without verified payment.
# ---------------------------------------------------------------------------
$enrollmentApi = Read-Lf $enrollmentApiPath
...
'@
    ...
    updated = crud_enrollment.update(
'@
    $enrollmentApi = Replace-Exact $enrollmentApi $needle $replacement 'premium activation guard'
}
...
'@
    ...
    $existing = Read-Lf $paymentsApiPath
    if (-not $existing.Contains('PAYMENT_INSTRUCTIONS')) {
        throw "payments.py already exists without TASK-7.1.1 instructions. Refusing overwrite."
    }
}
...
Write-Host "TASK-7.1.1 finished; changes remain uncommitted." -ForegroundColor Green
