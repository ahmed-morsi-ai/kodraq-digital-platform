...
$track = $track -replace 'from sqlalchemy import Boolean, Column, ForeignKey, Integer, String, Text',
...
'@
    $track = $track -replace '(?m)^(    ordering = Column\(Integer, default=0, nullable=False\)\r?\n)', ('$1' + $pricing)
}
if ($track -notmatch 'payments\s*=\s*relationship\("Payment"') {
    $track = $track -replace '(?m)^(    modules = relationship\()', ('    payments = relationship("Payment", back_populates="track", cascade="all, delete-orphan")' + "`r`n`r`n" + '$1')
}
...
if ($user -notmatch 'from app\.models\.payment import Payment') {
    $user = $user -replace '(?m)^    from app\.models\.ai import AIRequestLog\r?\n',
        ('    from app.models.ai import AIRequestLog' + "`r`n" + '    from app.models.payment import Payment' + "`r`n")
}
...
'@
    $user = $user -replace '(?m)^(    enrollments: Mapped\[list\[Enrollment\]\] = relationship\()', ($rel + "`r`n" + '$1')
}
...
if ($modelsInit -notmatch 'from app\.models\.payment import Payment') {
    $modelsInit = $modelsInit -replace '(?m)^from app\.models\.knowledge import ',
        ('from app.models.payment import Payment' + "`r`n" + 'from app.models.knowledge import ')
}
if ($modelsInit -notmatch '"Payment",') {
    $modelsInit = $modelsInit -replace '(?m)^    "KnowledgeDocument",\r?\n',
...
if ($crud -notmatch 'from app\.models\.track import Track') {
    $crud = $crud -replace '(?m)^from app\.models\.enrollment import Enrollment, StudentProgress\r?\n',
        ('from app.models.enrollment import Enrollment, StudentProgress' + "`r`n" + 'from app.models.track import Track' + "`r`n")
}
$oldBlock = @'
        db_obj = Enrollment(
            user_id=user_id,
